<script lang="ts">
  ...
  function loadRegions(): Record<Region, boolean> {
    if (typeof window === 'undefined') {
      return Object.fromEntries(FLAG_REGIONS.map((r) => [r, true])) as Record<Region, boolean>;
    }
  ...
  let anyRegionSelected = $derived(FLAG_REGIONS.some((r) => selectedRegions[r]));
...
{#if phase === 'setup'}
  <div class="min-h-screen bg-canvas text-fg flex flex-col">
    <ModeNav current="flag-attack" />
    <div class="flex-1 flex items-center justify-center p-6">
    <div class="max-w-md w-full">
      <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold">Flag Attack</h1>
      </div>

      <p class="text-muted mb-4">Select regions to include, then identify every country by its flag. You have 3 lives.</p>

      <div class="flex flex-wrap gap-2 mb-6">
        {#each FLAG_REGIONS as region}
          <button
